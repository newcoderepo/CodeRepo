...
def main(dataset, class_num, split_method, split_para, split_num, algorithm, modelname, loss, 
    ...
    post_fix_str = '{}_{}_{}_loss_{}_epoch_{}_{}'.format(algorithm, modelname, dataset,loss, local_epochs,num_glob_iters)
    model_path = []
    ...
    if dataset == 'Mnist':
        inputdim = 784
        model = linearModel(inputdim, 64, class_num).to(device), modelname
    ...
    for i in range(1): #repeat times
        if mode == 'personalisation': # for personlisaton test
            model_path = os.path.join("models", 'FedAVG')
            ...
            server = ServerMain(dataset, data, algorithm, model, batch_size, learning_rate, num_glob_iters, layer, fea_percent, fea_dim,
